...
class ProxyTestCommand:
    ...
    """
    def __init__(
        self, type, hostname, port_backend, port_proxy, username_backend, password_backend, username_proxy,
        password_proxy
    ):
        ...
        def callback_proxy(data):
            ...
            if not self.backend_data:
                ...
                if data == self.backend_data:
                    ...
                    self.deferred.errback(ValueError("Values from proxy and backend do not match!"))

        # execute exec command on both backend and proxy
        self.execute(
            self.hostname, self.port_backend, self.username_backend, self.password_backend, command, callback_backend
        )
        self.execute(self.hostname, self.port_proxy, self.username_proxy, self.password_proxy, command, callback_proxy)
