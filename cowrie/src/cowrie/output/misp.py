...
class Output(cowrie.core.output.Output):
    ...
    def find_attribute(self, attribute_type, searchterm):
        ...
        """
        result = self.misp_api.search(controller="attributes", type_attribute=attribute_type, value=searchterm)
    ...
    def add_sighting(self, entry, attribute):
        if self.is_python2:
            self.misp_api.sighting(uuid=attribute["uuid"], source="{} (Cowrie)".format(entry["sensor"]))
        else:
