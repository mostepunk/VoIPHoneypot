    ...
    def submit_entries(self, batch):
        ...
        for attempt in self.batch:
            log_output += u'{0}\t{1}\t{2}\t{3}\t{4}\t{5}\n'.format(
                attempt['date'], attempt['time'], attempt['timezone'], attempt['source_ip'], attempt['user'],
                attempt['password']
            )

        nonce = base64.b64decode(_nonceb64)
        digest = base64.b64encode(
            hmac.new(nonce + self.userid.encode('ascii'), base64.b64decode(self.auth_key), hashlib.sha256).digest()
        )
