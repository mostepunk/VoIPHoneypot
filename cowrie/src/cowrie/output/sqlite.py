    ...
    def write(self, entry):
        if entry["eventid"] == 'cowrie.session.connect':
            ...
            self.simpleQuery(
                ...
                "VALUES (?, ?, ?, ?)", (entry["session"], entry["timestamp"], sensorid, entry["src_ip"])
            )
            ...
            self.simpleQuery(
                ...
                'VALUES (?, ?, ?, ?, ?)',
                (entry["session"], 1, entry['username'], entry['password'], entry["timestamp"])
            )
            ...
            self.simpleQuery(
                ...
                'VALUES (?, ?, ?, ?, ?)',
                (entry["session"], entry["timestamp"], entry['url'], entry['outfile'], entry['shasum'])
            )

        elif entry["eventid"] == 'cowrie.session.file_download.failed':
            self.simpleQuery(
                'INSERT INTO `downloads` (`session`, `timestamp`, `url`, `outfile`, `shasum`) '
                'VALUES (?, ?, ?, ?, ?)', (entry["session"], entry["timestamp"], entry['url'], 'NULL', 'NULL')
            )

        elif entry["eventid"] == 'cowrie.session.file_download':
            self.simpleQuery(
                'INSERT INTO `input` (`session`, `timestamp`, `realm`, `input`) '
                'VALUES (?, ?, ?, ?)', (entry["session"], entry["timestamp"], entry["realm"], entry["input"])
            )

        elif entry["eventid"] == 'cowrie.client.version':
            r = yield self.db.runQuery('SELECT `id` FROM `clients` ' 'WHERE `version` = ?', (entry['version'], ))

            if r and r[0][0]:
                id = int(r[0][0])
            else:
                yield self.db.runQuery('INSERT INTO `clients` (`version`) ' 'VALUES (?)', (entry['version'], ))

                r = yield self.db.runQuery('SELECT LAST_INSERT_ROWID()')
                id = int(r[0][0])
            self.simpleQuery('UPDATE `sessions` ' 'SET `client` = ? ' 'WHERE `id` = ?', (id, entry["session"]))

        elif entry["eventid"] == 'cowrie.client.size':
            self.simpleQuery(
                'UPDATE `sessions` '
                'SET `termsize` = ? '
                'WHERE `id` = ?', ('%sx%s' % (entry['width'], entry['height']), entry["session"])
            )
