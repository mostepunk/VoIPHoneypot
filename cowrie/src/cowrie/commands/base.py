    ...
    def call(self):
        ...
        self.write(
            '%-8s pts/0    %s %s    0.00s  0.00s  0.00s w\n' % (
                self.protocol.user.username, self.protocol.clientIP[:17].ljust(17),
...
class command_who(HoneyPotCommand):
    def call(self):
        self.write(
            '%-8s pts/0        %s %s (%s)\n' % (
                self.protocol.user.username, time.strftime('%Y-%m-%d', time.localtime(self.protocol.logintime)),
                time.strftime('%H:%M', time.localtime(self.protocol.logintime)), self.protocol.clientIP
            )
        )
...
class command_ps(HoneyPotCommand):
    def call(self):
        ...
        output = (
            '%s'.ljust(15 - len("USER")) % "USER", '%s'.ljust(8 - len("PID")) % "PID", '%s'.ljust(13 - len("%CPU"))
            % "%CPU", '%s'.ljust(13 - len("%MEM")) % "%MEM", '%s'.ljust(12 - len("VSZ")) % "VSZ",
            '%s'.ljust(12 - len("RSS")) % "RSS", '%s'.ljust(10 - len("TTY")) % "TTY",
            '%s'.ljust(8 - len("STAT")) % "STAT", '%s'.ljust(8 - len("START")) % "START",
            '%s'.ljust(8 - len("TIME")) % "TIME", '%s'.ljust(30 - len("COMMAND")) % "COMMAND"
        )
        output_array.append(output)
        if (self.protocol.user.server.process):
            for single_ps in self.protocol.user.server.process:
                output = (
                    '%s'.ljust(15 - len(str(single_ps['USER']))) % str(single_ps['USER']),
            ...
            output = (
                '%s'.ljust(15 - len('root')) % 'root', '%s'.ljust(8 - len(str(process))) % str(process),
                '%s'.ljust(13 - len('0.0')) % '0.0', '%s'.ljust(13 - len('0.1')) % '0.1',
                '%s'.ljust(12 - len('5416')) % '5416', '%s'.ljust(12 - len('1024')) % '1024',
                '%s'.ljust(10 - len('?')) % '?', '%s'.ljust(8 - len('Ss')) % 'Ss',
                '%s'.ljust(8 - len('June22')) % 'June22', '%s'.ljust(8 - len('0:00')) % '0:00',
                '%s'.ljust(30 - len('/usr/sbin/sshd: %s@pts/0')) % '/usr/sbin/sshd: %s@pts/0' % user
            )
            output_array.append(output)
            process = process + 5
            output = (
                '%s'.ljust(15 - len(user)) % user, '%s'.ljust(8 - len(str(process))) % str(process),
                '%s'.ljust(13 - len('0.0')) % '0.0', '%s'.ljust(13 - len('0.1')) % '0.1', '%s'.ljust(12 - len('2925'))
                % '5416', '%s'.ljust(12 - len('1541')) % '1024', '%s'.ljust(10 - len('pts/0')) % 'pts/0',
                '%s'.ljust(8 - len('Ss')) % 'Ss', '%s'.ljust(8 - len('06:30')) % '06:30',
                '%s'.ljust(8 - len('0:00')) % '0:00', '%s'.ljust(30 - len('bash')) % '-bash'
            )
            output_array.append(output)
            process = process + 2
            output = (
                '%s'.ljust(15 - len(user)) % user, '%s'.ljust(8 - len(str(process))) % str(process),
                '%s'.ljust(13 - len('0.0')) % '0.0', '%s'.ljust(13 - len('0.1')) % '0.1', '%s'.ljust(12 - len('2435'))
                % '2435', '%s'.ljust(12 - len('929')) % '929', '%s'.ljust(10 - len('pts/0')) % 'pts/0',
                '%s'.ljust(8 - len('Ss')) % 'Ss', '%s'.ljust(8 - len('06:30')) % '06:30',
                '%s'.ljust(8 - len('0:00')) % '0:00', '%s'.ljust(30 - len('ps')) % 'ps %s' % ' '.join(self.args)
            )
...
class command_php(HoneyPotCommand):
    def start(self):
        if not len(self.args):
            ...
            output = (
                'Usage: php [options] [-f] <file> [--] [args...]', '       php [options] -r <code> [--] [args...]',
                '       php [options] [-B <begin_code>] -R <code> [-E <end_code>] [--] [args...]',
                '       php [options] [-B <begin_code>] -F <file> [-E <end_code>] [--] [args...]',
                '       php [options] -- [args...]', '       php [options] -a', '',
                '  -a               Run interactively', '  -c <path>|<file> Look for php.ini file in this directory',
                '  -n               No php.ini file will be used',
                "  -d foo[=bar]     Define INI entry foo with value 'bar'",
                '  -e               Generate extended information for debugger/profiler',
                '  -f <file>        Parse and execute <file>.', '  -h               This help',
                '  -i               PHP information', '  -l               Syntax check only (lint)',
                ...
                '  -s               Output HTML syntax highlighted source.', '  -v               Version number',
...
class command_nop(HoneyPotCommand):
    ...
    def exit(self):
        output, codec = self.get_output()
        realm = 'umask|unset|export|alias|jobs|kill|pkill|killall|killallS|su||chown|chattr|:|do|done'
        log.msg(
            eventid='cowrie.command.success',
            realm=realm,
            input=f"{realm} {' '.join(self.args)}" if self.args else realm,
