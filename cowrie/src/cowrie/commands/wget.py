...
class command_wget(HoneyPotCommand):
    ...
    def start(self):
        ...
        self.artifactFile.session = self.protocol.getProtoTransport().transportId
    ...
    def success(self, data, outfile):
        ...
        new_outfile = os.path.join(
            self.downloadPath, "session:" + self.artifactFile.session + ":" + outfile.split("/")[-1]
        )
        if not os.path.exists(new_outfile):
            os.rename(self.artifactFile.shasumFilename, new_outfile)
            duplicate = False
        else:
            os.remove(self.artifactFile.shasumFilename)
            duplicate = True
        ...
        if outfile != '-':
            ...
            self.fs.update_realfile(self.fs.getfile(outfile), new_outfile)
    ...
    def error(self, error, url):
        ...
        if hasattr(error, 'webStatus') and error.webStatus and hasattr(error, 'webMessage'):    # exceptions
            self.errorWrite(
                '{} ERROR {}: {}\n'.format(
                    time.strftime('%Y-%m-%d %T'), error.webStatus.decode(), error.webMessage.decode('utf8')
                )
            )
        else:
...
class HTTPProgressDownloader(client.HTTPDownloader):
            ...
            if self.totallength > 0:
                if not self.quiet:
                    self.wget.errorWrite(
                        'Length: {} ({}) [{}]\n'.format(
                            self.totallength, sizeof_fmt(self.totallength), self.contenttype
                        )
                    )
            else:
                if not self.quiet:
    ...
    def pageEnd(self):
        ...
        if not self.quiet:
            self.wget.errorWrite(
                '\r100%%[%s] %s %dK/s' %
                ('%s>' % (38*'='), splitthousands(str(int(self.totallength))).ljust(12), self.speed / 1000)
            )
            self.wget.errorWrite('\n\n')
            self.wget.errorWrite(
                '%s (%d KB/s) - `%s\' saved [%d/%d]\n\n' % (
                    time.strftime('%Y-%m-%d %H:%M:%S'), self.speed / 1000, self.fakeoutfile, self.currentlength,
                    self.totallength
                )
            )
        if self.fakeoutfile != '-':
