...
class command_curl(HoneyPotCommand):
    ...
    def start(self):
        try:
            ...
            # optlist, args = getopt.getopt(self.args, ['o', 'O'], ['help', 'manual', 'silent', 'remote-name', 'remote-name-all'])
...
Options: (H) means HTTP/HTTPS only, (F) means FTP only
 ...
 -q                 If used as the first parameter disables .curlrc\n"""
    ...
    def download(self, url, fakeoutfile, outputfile, *args, **kwargs):
        ...
        if scheme == 'https':
            contextFactory = ssl.ClientContextFactory(method=SSL.SSLv23_METHOD)
    ...
    def success(self, data, outfile):
        ...
        with open(self.safeoutfile, 'rb') as f:
            ...
            hashPath = os.path.join(
                self.download_path, "session:" + str(self.protocol.getProtoTransport().transportId) + ':' +
                (outfile.split("/")[-1] if outfile else shasum)
            )

        # If we have content already, delete temp file
        # or if file should not be saved
        if not os.path.exists(hashPath) and outfile:
...
class HTTPProgressDownloader(client.HTTPDownloader):
    ...
    def pageEnd(self):
        ...
        if self.fakeoutfile:
            ...
            # build hashPath again and update honeypot FS link to target real file

            # hashPath = os.path.join(self.curl.download_path, "session:" + str(self.curl.protocol.getProtoTransport().transportId) + ':' + self.fakeoutfile.split("/")[-1])
