...
class command_grep(HoneyPotCommand):
    ...
    def lineReceived(self, line):
        log.msg(eventid='cowrie.command.input', realm='grep', input=line, format='INPUT (%(realm)s): %(input)s')
...
class command_tail(HoneyPotCommand):
    ...
    def lineReceived(self, line):
        log.msg(eventid='cowrie.command.input', realm='tail', input=line, format='INPUT (%(realm)s): %(input)s')
...
class command_head(HoneyPotCommand):
    ...
    def lineReceived(self, line):
        log.msg(eventid='cowrie.command.input', realm='head', input=line, format='INPUT (%(realm)s): %(input)s')
    ...
    def call(self):
        ...
        for f in args:
            ...
            try:
                ...
                if not force:
                    self.errorWrite('rm: cannot remove `{}\': No such file or directory\n'.format(f))
                continue
...
class command_cp(HoneyPotCommand):
    ...
    def call(self):
        ...
        if dest[-1] == '/' and not self.fs.exists(resolv(dest)) and \
                not recursive:
            self.errorWrite("cp: cannot create regular file `{}': Is a directory\n".format(dest))
            return
        ...
        for src in sources:
            if not self.fs.exists(resolv(src)):
                self.errorWrite("cp: cannot stat `{}': No such file or directory\n".format(src))
                continue
...
class command_mv(HoneyPotCommand):
    ...
    def call(self):
        ...
        if dest[-1] == '/' and not self.fs.exists(resolv(dest)) and len(sources) != 1:
            self.errorWrite("mv: cannot create regular file `{}': Is a directory\n".format(dest))
            return
        ...
        for src in sources:
            if not self.fs.exists(resolv(src)):
                self.errorWrite("mv: cannot stat `{}': No such file or directory\n".format(src))
                continue
    ...
    def call(self):
        for f in self.args:
            ...
            if self.fs.exists(pname):
                self.errorWrite('mkdir: cannot create directory `{}\': File exists\n'.format(f))
                return
            try:
        ...
        for f in self.args:
            ...
            try:
                if len(self.fs.get_path(pname)):
                    self.errorWrite('rmdir: failed to remove `{}\': Directory not empty\n'.format(f))
                    continue
                dir = self.fs.get_path('/'.join(pname.split('/')[:-1]))
            except (IndexError, fs.FileNotFound):
                dir = None
            ...
            if not dir or fname not in [x[fs.A_NAME] for x in dir]:
                self.errorWrite('rmdir: failed to remove `{}\': No such file or directory\n'.format(f))
                continue
            for i in dir[:]:
    ...
    def call(self):
        ...
        for f in self.args:
            ...
            if not self.fs.exists(os.path.dirname(pname)):
                self.errorWrite('touch: cannot touch `{}`: No such file or directory\n'.format(pname))
                return
            if self.fs.exists(pname):
            ...
            if any([pname.startswith(_p) for _p in fs.SPECIAL_PATHS]):
                self.errorWrite('touch: cannot touch `{}`: Permission denied\n'.format(pname))
                return
