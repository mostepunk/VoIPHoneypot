...
Options:
  ...
  Plugin Options:
    ...
    def do_install(self, *args):
        ...
        for p in packages:
            self.write(
                '---> Package {0}.{1} {2}.{3} will be installed\n'.format(
        ...
        for p in packages:
            self.write(
                ' {0}\t\t\t\t{1}\t\t\t{2}-{3}\t\t\t{4}\t\t\t\t{5} k\n'.format(
                    p, arch, packages[p]['version'], packages[p]['release'], repository, packages[p]['size']
                )
            )
        self.write('\n')
    ...
    def do_locked(self):
        self.errorWrite('Loaded plugins: changelog, kernel-module, ovl, priorities, tsflags, versionlock\n')
