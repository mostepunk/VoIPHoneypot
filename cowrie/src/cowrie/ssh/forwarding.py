    ...
    if tunnelEnabled:
        ...
        if remoteHP[1] in tunnels:
            ...
            return TCPTunnelForwardingChannel(
                remoteHPNew, remoteHP, remoteWindow=remoteWindow, remoteMaxPacket=remoteMaxPacket
            )
...
class TCPTunnelForwardingChannel(forwarding.SSHConnectForwardingChannel):
    ...
    def dataReceived(self, data):
        log.msg(eventid='cowrie.tunnelproxy-tcpip.data', format='sending via tunnel proxy %(data)s', data=repr(data))
