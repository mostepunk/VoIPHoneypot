...
class HoneyPotSSHTransport(transport.SSHServerTransport, TimeoutMixin):
    ...
    def ssh_KEXINIT(self, packet):
        ...
        strings, _ = k[:-1], k[-1]
        (kexAlgs, keyAlgs, encCS, _, macCS, _, compCS, _, langCS, _) = [s.split(b',') for s in strings]
    ...
    def connectionLost(self, reason):
        ...
        log.msg(eventid='cowrie.session.closed', format="Connection lost after %(duration)d seconds", duration=duration)
