...
class HoneyPotCommand(object):
    ...
    def __init__(self, protocol, *args):
        ...
        self.temp_buff = []    # used to store intercepted output
        self.codec = "plain"    # log format (plain=string, base64=base64 encoded)
    ...
    def write(self, data):
        ...
        """
        self.temp_buff.append(data)
        return self.writefn(data.encode('utf-8'))
    ...
    def get_output(self):
        if self.codec == "plain":
            try:
                return "\n".join(self.temp_buff), self.codec
            except:
                return "[Router]: Error in concatination", self.codec
        else:
            try:
                buffer = self.temp_buff[0]

                if len(self.temp_buff) > 1:
                    for out in self.temp_buff[1:]:
                        if isinstance(out, "str"):
                            buffer += out.encode()
                        else:
                            buffer += out

                return base64.b64encode(buffer).decode("utf-8"), self.codec
            except Exception:
                self.codec = "plain"
                return "[Router]: Error in encoding!", self.codec
