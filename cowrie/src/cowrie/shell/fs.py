...
class HoneyPotFilesystem(object):
    ...
    def open(self, filename, openFlags, mode):
        ...
        if openFlags & os.O_WRONLY == os.O_WRONLY or openFlags & os.O_RDWR == os.O_RDWR:
            ...
            hostfile = '%s/%s_sftp_%s' % (
                CowrieConfig().get('honeypot', 'download_path'), time.strftime('%Y%m%d-%H%M%S'),
    ...
    def close(self, fd):
        ...
        if self.tempfiles[fd] is not None:
            ...
            shasumfile = CowrieConfig().get('honeypot', 'download_path') + "/" + self.session + '.' + shasum
            fname = os.path.basename(self.filenames[fd])
            new_filepath = CowrieConfig().get('honeypot', 'download_path') + "/session:" + self.session + ':' + fname
            if (os.path.exists(new_filepath)):
                os.remove(self.tempfiles[fd])
            else:
                os.rename(self.tempfiles[fd], new_filepath)
            self.update_realfile(self.getfile(self.filenames[fd]), new_filepath)
            log.msg(
                format='SFTP Uploaded file \"%(filename)s\" to %(outfile)s',
                eventid='cowrie.session.file_upload',
                filename=fname,
                outfile=new_filepath,
