...
class LibvirtBackendService:
    def __init__(self):
        ...
        if self.conn is None:
            log.msg(eventid='cowrie.backend_pool.qemu', format='Failed to open connection to qemu:///system')
            raise LibvirtError()
        ...
        log.msg(eventid='cowrie.backend_pool.qemu', format='Connection to Qemu established')
    ...
    def stop_backend(self):
        log.msg(eventid='cowrie.backend_pool.qemu', format='Doing Qemu clean shutdown...')
    ...
    def shutdown_backend(self):
        ...
        log.msg(eventid='cowrie.backend_pool.qemu', format='Connection to Qemu closed successfully')
    ...
    def destroy_guest(self, domain, snapshot):
        ...
        try:
            ...
            log.err(eventid='cowrie.backend_pool.qemu', format='Error destroying guest: %(error)s', error=error)

    def __destroy_all_guests(self):
        domains = self.conn.listDomainsID()
        if not domains:
            log.msg(eventid='cowrie.backend_pool.qemu', format='Could not get domain list')
    ...
    def __destroy_all_networks(self):
        ...
        if not networks:
            log.msg(eventid='cowrie.backend_pool.qemu', format='Could not get network list')
    ...
    def __destroy_all_network_filters(self):
        ...
        if not network_filters:
            log.msg(eventid='cowrie.backend_pool.qemu', format='Could not get network filters list')
