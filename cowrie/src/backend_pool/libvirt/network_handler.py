...
def create_filter(connection):
    ...
    try:
        ...
        log.err(eventid='cowrie.backend_pool.network_handler', format='Filter already exists: %(error)s', error=e)
...
def create_network(connection, network_table):
    ...
    try:
        ...
        if net is None:
            log.msg(eventid='cowrie.backend_pool.network_handler', format='Failed to define a virtual network')
            exit(1)
        ...
        log.err(eventid='cowrie.backend_pool.network_handler', format='Network already exists: %(error)s', error=e)
